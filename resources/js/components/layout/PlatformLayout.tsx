...
export function PlatformLayout({ children }: { children: ReactNode }) {
    ...
    const sidebar = <div className="flex h-full flex-col">
        ...
        <nav className="flex-1 space-y-1 p-3">
            ...
            <Link href="/dashboard/" className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium text-slate-300 transition hover:bg-white/5 hover:text-white"><LayoutDashboard size={18}/><span>Hotel dashboard</span></Link>
        </nav>
    ...
    return <div className="min-h-screen w-full bg-[#f4f6f9] text-left text-slate-900">
        ...
        <div className="relative min-h-screen bg-[#f4f6f9] lg:ml-72">
            <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/90 px-5 py-3 backdrop-blur lg:px-8"><div className="mx-auto flex max-w-[1180px] items-center justify-between"><div className="flex items-center gap-3"><button onClick={() => setMobileOpen(true)} className="rounded-xl border border-slate-200 p-2.5 text-slate-700 lg:hidden"><Menu size={20}/></button><div><p className="text-sm font-bold text-slate-900">Platform administration</p><p className="hidden text-xs text-slate-500 sm:block">Manage your client properties and subscriptions</p></div></div><Link href="/dashboard/" className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50"><LayoutDashboard size={16}/><span className="hidden sm:inline">Hotel dashboard</span></Link></div></header>
